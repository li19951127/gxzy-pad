angular.module('starter.twoLotteryFootball', [])
    .controller('twoLotteryFootballCtrl', function ($scope, $state, $util, $rootScope, $interval, $stateParams, BettingService,$getActivityData, $errorPopupFactory) {
        ...
        console.log("2串1数据", $scope.plandata);
        var userId = $rootScope.userId;
        console.log("userId", userId);
        $scope.oneTeamData = $scope.plandata[0]; //第一场数据
        $scope.twoTeamData = $scope.plandata[1]; //第二场数据
        $scope.twoScheme = $scope.plandata[2]; //2串1方案
        ...
        $scope.dataChange = function (data) {
            if (eval(data.join('+')) > 1) {
                ...
                $scope.money = $scope.noteNum * parseInt($scope.noteOne*$scope.saleNum) ;
            } else {
                $scope.bonus = 0;
                $scope.noteNum = 0;
                $scope.money = 0;
        ...
        $scope.showOrderAlertCms = function () {
            ...
            if ($scope.oneBetnum[0] === $scope.oneBetnum[1] || $scope.twoBetnum[0] === $scope.twoBetnum[1]) {
                $scope.successOrFaild = '两场比赛至少各选种一赛果!';
                $scope.imgagesUrl = imgClass[1];
